...
class DatosF987 extends CI_Controller{


	public function __construct(){
		parent::__construct();
		$this->load->helper("url");
		$this->load->model("DatosF987_Model");
	...
	public function index(){
		$dataBitacora = array(
			"idAccion" => 03,
			"descripcion" => "Usuario ".$_SESSION['usuario']." ingresó al Modulo de Datos F987.",
		...
		$this->load->view("Reportes/DatosF987");
		$this->load->view("layout/footer");
		$this->load->view("layout/end_footer");
		$this->load->view("Reportes/end_DatosF987");
	}

	public function getF987(){
		$fechaInicio = date("d/m/Y",strtotime($this->input->post("FechaInicio")));
		$fechaFin = date("d/m/Y",strtotime($this->input->post("FechaFin")));
		$res = $this->DatosF987_Model->getF987($fechaInicio, $fechaFin);
		if (COUNT($res) > 0){
			//Cargamos la librería de excel.
			$this->load->library('excel');
			$this->excel->setActiveSheetIndex(0);
			$this->excel->getActiveSheet()->setCellValue("A1", 'DATOS PARA F987 DEL '.$fechaInicio.' AL '.$fechaFin.'.');
			$this->excel->getActiveSheet()->setTitle('DATOS F987');
			$this->excel->setActiveSheetIndex(0)->mergeCells('A1:U1');
			//Contador de filas
			$contador = 2;
			...
			$this->excel->getActiveSheet()->getStyle('A1:V1')->applyFromArray($estiloTituloReporte);
			$this->excel->setActiveSheetIndex(0)->mergeCells('A1:V1');
			//Contador de filas
			$contador = 2;
			...
			//Le aplicamos negrita a los títulos de la cabecera.
			$this->excel->getActiveSheet()->getStyle("A{$contador}:V{$contador}")->getFont()->setBold(true);
			//Le aplicamos color a los titulos.
			$this->excel->getActiveSheet()->getStyle("A{$contador}:V{$contador}")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('70AD47');
			...
			$this->excel->getActiveSheet()->setCellValue("M{$contador}", 'CALLE/AVENIDA/PASAJE/POLÍGONO/BLOCK');
			$this->excel->getActiveSheet()->setCellValue("N{$contador}", 'N° CASA');
			$this->excel->getActiveSheet()->setCellValue("O{$contador}", 'APARTAMENTO/LOCAL');
			$this->excel->getActiveSheet()->setCellValue("P{$contador}", 'OTROS');
			$this->excel->getActiveSheet()->setCellValue("Q{$contador}", 'COLONIA/BARRIO/RESIDENCIAL/REPARTO');
			$this->excel->getActiveSheet()->setCellValue("R{$contador}", 'CORREO ELECTRONICO');
			$this->excel->getActiveSheet()->setCellValue("S{$contador}", 'DEPARTAMENTO');
			$this->excel->getActiveSheet()->setCellValue("T{$contador}", 'MUNICIPIO');
			$this->excel->getActiveSheet()->setCellValue("U{$contador}", 'TELEFONO');
			...
			foreach ($res as $d) {
				...
				$this->excel->getActiveSheet()->setCellValue("M{$contador}", $d->direccion);
				$this->excel->getActiveSheet()->setCellValue("N{$contador}", $d->noCasa);
				$this->excel->getActiveSheet()->setCellValue("O{$contador}", $d->aptoLocal);
				$this->excel->getActiveSheet()->setCellValue("P{$contador}", $d->otrosDatos);
				$this->excel->getActiveSheet()->setCellValue("Q{$contador}", $d->colonia);
				$this->excel->getActiveSheet()->setCellValue("R{$contador}", $d->correo);
				$this->excel->getActiveSheet()->setCellValue("S{$contador}", $d->depto);
				$this->excel->getActiveSheet()->setCellValue("T{$contador}", $d->municipio);
				$this->excel->getActiveSheet()->setCellValue("U{$contador}", $d->telefono);
				$this->excel->getActiveSheet()->setCellValue("V{$contador}", $d->usuCrea);
			}
			...
			$dataBitacora = array(
				"idAccion" => 15,
				"descripcion" => "Usuario ".$_SESSION['usuario']." Descargó reporte de Datos F987 de ".date("d/m/Y",strtotime($fechaInicio))." al ".date("d/m/Y",strtotime($fechaFin))." desde la IP ".$_SERVER['REMOTE_ADDR']." y equipo ".gethostbyaddr($_SERVER['REMOTE_ADDR']).".",
				"usuario" => $_SESSION['usuario'],
				"dirIp"=>$_SERVER['REMOTE_ADDR'],
				"nomMaquina"=>gethostbyaddr($_SERVER['REMOTE_ADDR'])
			);
	...
	public function ObtenerPermiso(){
		$permiso=$this->GestionPermiso_Model->getPermiso($_SESSION['usuario'],'12');
