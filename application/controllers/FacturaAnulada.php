...
class FacturaAnulada extends CI_Controller{

	public function __construct(){
		parent::__construct();
		$this->load->helper("url");
		$this->load->model("FacturaAnulada_Model");
	...
	public function index(){
		...
		$this->load->view("Reportes/FacturaAnulada");
...
//		$this->load->view("Reportes/end_FacturasGeneradas");
	...
	public function getFacturas(){
		...
		$res = $this->FacturaAnulada_Model->getFacturas($fechaInicio, $fechaFin);
		if (COUNT($res) > 0){
			//Cargamos la librería de excel.
			$this->load->library('excel');
			$this->excel->setActiveSheetIndex(0);
			$this->excel->getActiveSheet()->setCellValue("A1", 'FACTURAS ANULADAS DEL '.$fechaInicio.' AL '.$fechaFin.'.');
			$this->excel->getActiveSheet()->setTitle('FACTURAS ANULADAS');
			...
			$this->excel->getActiveSheet()->getStyle("A{$contador}:L{$contador}")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('70AD47');
			...
			$this->excel->getActiveSheet()->setCellValue("F{$contador}", 'CORRELATIVO PRELIMINAR');
			...
			$this->excel->getActiveSheet()->setCellValue("J{$contador}", 'CORRELATIVO ANULACIÓN');
			$this->excel->getActiveSheet()->setCellValue("K{$contador}", 'MOTIVO ANULACIÓN');
			$this->excel->getActiveSheet()->setCellValue("L{$contador}", 'USUARIO ANULACIÓN');
			foreach ($res as $d) {
				...
				$this->excel->getActiveSheet()->setCellValue("F{$contador}", $d->corrPrel);
				$this->excel->getActiveSheet()->setCellValue("G{$contador}", $d->corrDefi);
				$this->excel->getActiveSheet()->setCellValue("H{$contador}", $d->Tipo);
				$this->excel->getActiveSheet()->setCellValue("I{$contador}", $d->Estados);
				$this->excel->getActiveSheet()->setCellValue("J{$contador}", $d->corAnul);
				$this->excel->getActiveSheet()->setCellValue("K{$contador}", $d->motivoAnul);
				$this->excel->getActiveSheet()->setCellValue("L{$contador}", $d->usuAnul);
			}
			//Le ponemos un nombre al archivo que se va a generar.
			$archivo = "FacturasAnuladas.xls";
			...
			$dataBitacora = array(
				"idAccion" => 15,
				"descripcion" => "Usuario ".$_SESSION['usuario']." Descargó reporte de Facturas Anuladas de la semana del ".date("d/m/Y",strtotime($fechaInicio))." al ".date("d/m/Y",strtotime($fechaFin))." desde la IP ".$_SERVER['REMOTE_ADDR']." y equipo ".gethostbyaddr($_SERVER['REMOTE_ADDR']).".",
				"usuario" => $_SESSION['usuario'],
				"dirIp"=>$_SERVER['REMOTE_ADDR'],
				"nomMaquina"=>gethostbyaddr($_SERVER['REMOTE_ADDR'])
			);
	...
	public function ObtenerPermiso(){
		$permiso=$this->GestionPermiso_Model->getPermiso($_SESSION['usuario'],'30');
